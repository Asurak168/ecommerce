...
const create = async (req, res) => {
  ...
  try {
    const findUser = await conn.query("SELECT * FROM employee WHERE Tel = ?", [
      Tel,
    ]);
    if (findUser.length)
      return res.json({ msg: "This Telephone is already used!!" });
    const sql =
      "INSERT INTO employee (Firstname, Lastname, Gender, Dob, Tel, Address, Email) VALUES (?,?,?,?,?,?,?)";
    try {
...
const getOne = async (req, res) => {
  const { id } = req.params;
  try {
    const data = await conn.query("SELECT * FROM employee WHERE id = ?", [id]);
    res.json({ List: data });
  } catch (error) {
    res.json({ ERROR: error });
  }
...
const updateOne = async (req, res) => {
  ...
  try {
    ...
    if (findUser.length)
      return res.json({ msg: "This Telephone is already used!!" });
    const sql =
      "UPDATE employee SET Email = ? Firstname = ?, Lastname = ?, Gender = ?, Dob = ?, Tel = ?, Address = ?) WHERE id = ?";
    try {
      const newUser = await conn.query(sql, [
        Email,
        Firstname,
...
const deleteOne = async (req, res) => {
  const { id } = req.params;
  try {
    const data = await conn.query("DELETE FROM employee WHERE id = ?", [id]);
    res.json(
      data.affectedRows !== 0
